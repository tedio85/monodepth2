...
import model_components.networks as networks

from collections import defaultdict
from rotm2euler import *
from test_utils import *
from experiment_utils import *
from model_components.layers import *
...
def read_pair_list(pair_path):
    """ example line: 2011_09_26/2011_09_26_drive_0022_sync 473 r """
    f = open(pair_path)
    lines = f.readlines()
    ...
    for ln in lines:
        ...
        scn = split[0]
        tgt_id, side = int(split[1]), int(split[2])
        scn = os.path.join(scn, "image_0{}/data".format(side_map[side]))

        src_id0 = tgt_id - 1
        tup = (scn, tgt_id, src_id0)
        frames.append(tup)

        src_id1 = tgt_id + 1
        tup = (scn, tgt_id, src_id1)
        frames.append(tup)
...
def load_data(record, root, img_width=320, img_height=240):
    ...
    def load_image(data_root, scn, fid, w, h):
        img_path = get_image_path(data_root, scn, fid)
    ...
    def load_intrinsics(img_h, img_w):
    ...
    data = dict()
    data['tgt'] = load_image(root['data'], scn, tgt_id, img_width, img_height)
    data['src'] = load_image(root['data'], scn, src_id, img_width, img_height)
    data['K'] = load_intrinsics(img_height, img_width)
...
def get_model_dict(ckpt_path):
    models = dict()

    # DEPTH network
    models["encoder"] = networks.ResnetEncoder(num_layers=18, pretrained=True)
    models["encoder"].cuda()
    models["encoder"].eval()
    ...
    models["pose_encoder"] = networks.ResnetEncoder(
                                ...
                                pretrained=True,
                                num_input_images=2)
    models["pose_encoder"].cuda()
    models["pose_encoder"].eval()
    ...
    assert os.path.isdir(ckpt_path), "Cannot find folder {}".format(ckpt_path)
    print("loading model from folder {}".format(ckpt_path))

    for n in models.keys():
        print("Loading {} weights...".format(n))
        path = os.path.join(ckpt_path, "{}.pth".format(n))
...
def run_test(frame_t,
    ...
    features = models["encoder"](frame['tgt']))
...
if __name__ == '__main__':
    ...
    list_path = "./model_components/splits/eigen_zhou/train_files.txt"
    root = dict()
    root['data'] = "/viscompfs/users/tedyu/kitti_data/kitti"
    root['dump'] = "/viscompfs/users/sawang/ScanNet/analysis_clip_params_kitti"
    root['ckpt'] = "/viscompfs/users/sawang/checkpoints/mono_motion_range/only_depth/default/mono_model/models/weights_19"
    img_w, img_h = 320, 240
    deviation = 0.2
    step = 5e-3
    ...
    # initialize models
    models = get_model_dict(root['ckpt'])
